...
class IndexController extends ControllerBase
{
	...
	if ($id) {
	    $config = Configuration::getDefaultConfiguration()->setHost($this->config->kratos->api_host);
	    $client = new Client;
	...
	$after_verification_return_to = $this->request->get('after_verification_return_to');
	...
	$redirect_url = getUrlForFlow($this->config->kratos->api_host, 'registration', $params);
	...
	$config = Configuration::getDefaultConfiguration()->setHost($this->config->kratos->browser_host);
	$client = new Client;
	$frontend_api = new FrontendApi($client, $config);
	try {
